...
contract AbsToken is IERC20, Ownable {
    ...
    string public _name = "ARK";
    string public _symbol = "ARK";
    uint8 public _decimals = 18;
    mapping(address => bool) public _feeWhiteList;
    mapping(address => bool) public _blackList;
    mapping(address => bool) public _lockAddressList;
    address public _swapPair;
    uint256 public _tTotal = 21000 * (10 ** _decimals);
    ...
    mapping(address => bool) public _isExcludedContract;
            ...
            if (_swapPairList[to] && (startTradeBlock != 0)) {
                if ((!inSwap) && (!isAdd)) {
                    ...
                    if (contractTokenBalance > threshold) {
                        uint256 swapFee = ((_buyLPFee + _buyLPDividendFee) + _sellLPDividendFee) + _sellLPFee;
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isSell, bool isTransfer) private {
        _balances[sender] = _balances[sender] - tAmount;
        uint256 feeAmount;
        if (takeFee) {
            uint256 swapFee;
    ...
    function swapTokenForFund(uint256 tokenAmount, uint256 swapFee) private lockTheSwap() {
        ...
        WETH.transferFrom(address(_tokenDistributor), address(this), wethBalance);
        if (lpAmount > 0) {
            uint256 lpWeth = (wethBalance * lpFee) / swapFee;
            if (lpWeth > 0) {
                _swapRouter.addLiquidity(address(this), _weth, lpAmount, lpWeth, 0, 0, receiveAddress, block.timestamp);
            }
        }
    }
    ...
    function multiTransfer4AirDrop(address[] calldata addresses, uint256 tokens) external onlyOwner() {
