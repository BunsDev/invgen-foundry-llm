...
contract BlackGold is Context, IBEP20, Ownable {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal {
        ...
        if (!_isExcludedFromFee[sender]) {
            require(_balances[sender] > amount.add(miningFee).add(liquidityFee).add(burnFee), "BEP20: transfer amount + fees exceeds balance");
        }
    ...
    function _burn(address account, uint256 amount) internal {
        require(amount != 0);
        require(amount <= _totalSupply, "BEP20: Burn amount exceeds _totalSupply");
    ...
    function _findOnePercent(uint256 value) public pure returns (uint256) {
