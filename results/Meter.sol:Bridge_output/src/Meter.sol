...
abstract contract AccessControl is Context {
    ...
    function grantRole(bytes32 role, address account) virtual public {
    ...
    ///      * Requirements:
    ///      * - the caller must have ``role``'s admin role.
    function revokeRole(bytes32 role, address account) virtual public {
    ...
    function renounceRole(bytes32 role, address account) virtual public {
...
contract Pausable {
    ...
    function _whenPaused() public view {
...
contract SafeMath {
    ...
    ///      * Requirements:
    /// - Subtraction cannot overflow.
    function _sub(uint256 a, uint256 b, string memory errorMessage) public pure returns (uint256) {
...
interface IGenericHandler {
    ...
    function setResource(bytes32 resourceID, address contractAddress, bytes4 depositFunctionSig, uint depositFunctionDepositerOffset, bytes4 executeFunctionSig) external;
}
...
contract Bridge is Pausable, AccessControl, SafeMath {
    ...
    event ProposalEvent(uint8 indexed originChainID, uint64 indexed depositNonce, ProposalStatus indexed status, bytes32 resourceID, bytes32 dataHash);
    ...
    function _onlyAdminOrOperator() private {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender) || hasRole(OPERATOR_ROLE, msg.sender), "sender is not operator or admin");
    }
    ...
    /// @param resourceID ResourceID to be used when making deposits.
    /// @param contractAddress Address of contract to be called when a deposit is made and a deposited is executed.
    function adminSetGenericResource(address handlerAddress, bytes32 resourceID, address contractAddress, bytes4 depositFunctionSig, uint256 depositFunctionDepositerOffset, bytes4 executeFunctionSig) external onlyAdmin() {
        _resourceIDToHandlerAddress[resourceID] = handlerAddress;
    ...
    /// @return _fee
    function _getFee(uint8 destinationChainID) public view returns (uint256) {
    ...
    function adminUpdateBridgeAddress(address handlerAddress, address newBridgeAddress) external onlyAdmin() {
        IERCHandler handler = IERCHandler(handlerAddress);
    ...
    function adminWithdraw(address handlerAddress, address tokenAddress, address recipient, uint256 amountOrTokenID) external onlyAdmin() {
        IERCHandler handler = IERCHandler(handlerAddress);
    ...
    function voteProposal(uint8 chainID, uint64 depositNonce, bytes32 resourceID, bytes32 dataHash) external onlyRelayers() whenNotPaused() {
        ...
        if (uint(proposal._status) == 0) {
            ++_totalProposals;
            _proposals[nonceAndID][dataHash] = Proposal({_resourceID: resourceID, _dataHash: dataHash, _yesVotes: new address[](1), _noVotes: new address[](0), _status: ProposalStatus.Active, _proposedBlock: block.number});
