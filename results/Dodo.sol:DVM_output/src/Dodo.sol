...
library DODOMath {
    ...
    function _SolveQuadraticFunctionForTarget(uint256 V1, uint256 delta, uint256 i, uint256 k) public pure returns (uint256) {
        if (V1 == 0) {
            return 0;
        }
        if (k == 0) {
    ...
    function _SolveQuadraticFunctionForTrade(uint256 V0, uint256 V1, uint256 delta, uint256 i, uint256 k) public pure returns (uint256) {
        require(V0 > 0, "TARGET_IS_ZERO");
        if (delta == 0) {
            return 0;
        }
        ...
        uint256 squareRoot = DecimalMath.mulFloor(DecimalMath.ONE.sub(k).mul(4), DecimalMath.mulFloor(k, V0).mul(V0));
...
library PMMPricing {
    ...
    function sellBaseToken(PMMState memory state, uint256 payBaseAmount) public pure returns (uint256 receiveQuoteAmount, RState newR) {
        if (state.R == RState.ONE) {
            ...
            if (payBaseAmount < backToOnePayBase) {
                ...
                receiveQuoteAmount = backToOneReceiveQuote.add(_ROneSellBaseToken(state, payBaseAmount.sub(backToOnePayBase)));
                newR = RState.BELOW_ONE;
            }
        } else {
    ...
    function sellQuoteToken(PMMState memory state, uint256 payQuoteAmount) public pure returns (uint256 receiveBaseAmount, RState newR) {
        if (state.R == RState.ONE) {
            ...
            if (payQuoteAmount < backToOnePayQuote) {
                ...
                receiveBaseAmount = backToOneReceiveBase.add(_ROneSellQuoteToken(state, payQuoteAmount.sub(backToOnePayQuote)));
                newR = RState.ABOVE_ONE;
            }
        }
    }

    function _ROneSellBaseToken(PMMState memory state, uint256 payBaseAmount) public pure returns (uint256) {
        return DODOMath._SolveQuadraticFunctionForTrade(state.Q0, state.Q0, payBaseAmount, state.i, state.K);
    }

    function _ROneSellQuoteToken(PMMState memory state, uint256 payQuoteAmount) public pure returns (uint256) {
        return DODOMath._SolveQuadraticFunctionForTrade(state.B0, state.B0, payQuoteAmount, DecimalMath.reciprocalFloor(state.i), state.K);
    }

    function _RBelowSellQuoteToken(PMMState memory state, uint256 payQuoteAmount) public pure returns (uint256) {
        return DODOMath._GeneralIntegrate(state.Q0, state.Q.add(payQuoteAmount), state.Q, DecimalMath.reciprocalFloor(state.i), state.K);
    }

    function _RBelowSellBaseToken(PMMState memory state, uint256 payBaseAmount) public pure returns (uint256) {
        return DODOMath._SolveQuadraticFunctionForTrade(state.Q0, state.Q, payBaseAmount, state.i, state.K);
    }

    function _RAboveSellBaseToken(PMMState memory state, uint256 payBaseAmount) public pure returns (uint256) {
        return DODOMath._GeneralIntegrate(state.B0, state.B.add(payBaseAmount), state.B, state.i, state.K);
    }

    function _RAboveSellQuoteToken(PMMState memory state, uint256 payQuoteAmount) public pure returns (uint256) {
        return DODOMath._SolveQuadraticFunctionForTrade(state.B0, state.B, payQuoteAmount, DecimalMath.reciprocalFloor(state.i), state.K);
    }
    ...
    function adjustedTarget(PMMState memory state) public pure {
        if (state.R == RState.BELOW_ONE) {
            ...
            state.B0 = DODOMath._SolveQuadraticFunctionForTarget(state.B, state.Q.sub(state.Q0), DecimalMath.reciprocalFloor(state.i), state.K);
        }
    }
...
contract DVMVault is DVMStorage {
    ...
    function _setReserve(uint256 baseReserve, uint256 quoteReserve) internal {
        require((baseReserve <= type(uint128).max) && (quoteReserve <= type(uint128).max), "OVERFLOW");
    ...
    function _sync() internal {
        ...
        require((baseBalance <= type(uint128).max) && (quoteBalance <= type(uint128).max), "OVERFLOW");
...
interface IDODOCallee {
    function DVMSellShareCall(address sender, uint256 burnShareAmount, uint256 baseAmount, uint256 quoteAmount, bytes calldata data) external;
...
contract DVMTrader is DVMVault {
    ...
    function flashLoan(uint256 baseAmount, uint256 quoteAmount, address assetTo, bytes calldata data) external preventReentrant() {
        ...
        if (baseBalance < _BASE_RESERVE_) {
            ...
            _transferBaseOut(_MAINTAINER_, mtFee);
            emit DODOSwap(address(_QUOTE_TOKEN_), address(_BASE_TOKEN_), quoteInput, receiveBaseAmount, msg.sender, assetTo);
        }
        if (quoteBalance < _QUOTE_RESERVE_) {
            uint256 baseInput = baseBalance.sub(uint256(_BASE_RESERVE_));
            ...
            _transferQuoteOut(_MAINTAINER_, mtFee);
            emit DODOSwap(address(_BASE_TOKEN_), address(_QUOTE_TOKEN_), baseInput, receiveQuoteAmount, msg.sender, assetTo);
        }
        _sync();
...
contract DVMFunding is DVMVault {
    ...
    function buyShares(address to) external preventReentrant() returns (uint256 shares, uint256 baseInput, uint256 quoteInput) {
        uint256 baseBalance = _BASE_TOKEN_.balanceOf(address(this));
        uint256 quoteBalance = _QUOTE_TOKEN_.balanceOf(address(this));
    ...
    function sellShares(uint256 shareAmount, address to, uint256 baseMinAmount, uint256 quoteMinAmount, bytes calldata data, uint256 deadline) external preventReentrant() returns (uint256 baseAmount, uint256 quoteAmount) {
...
contract DVM is DVMTrader, DVMFunding {
    ...
    function init(address maintainer, address baseTokenAddress, address quoteTokenAddress, uint256 lpFeeRate, address mtFeeRateModel, uint256 i, uint256 k, bool isOpenTWAP) external {
        ...
        DOMAIN_SEPARATOR = keccak256(abi.encode(0x8b73c3c69bb8fe3d512ecc4cf759cc79239f7b179b0ffacaa9a75d522b39400f, keccak256(bytes(name)), keccak256(bytes("1")), chainId, address(this)));
    }
