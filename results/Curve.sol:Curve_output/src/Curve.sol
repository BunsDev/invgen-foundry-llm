...
library Curves {
    ...
    function add(uint256 x, uint256 y, string memory errorMessage) public pure returns (uint256 z) {
        require((z = x + y) >= x, errorMessage);
    }

    function sub(uint256 x, uint256 y, string memory errorMessage) public pure returns (uint256 z) {
        require((z = x - y) <= x, errorMessage);
    }
    ...
    ///  `amount`
    function transferFrom(Storage.Curve storage curve, address sender, address recipient, uint256 amount) external returns (bool) {
        _transfer(curve, sender, recipient, amount);
        _approve(curve, sender, msg.sender, sub(curve.allowances[sender][msg.sender], amount, "Curve/insufficient-allowance"));
        return true;
    }
    ...
    ///  Requirements:
    ///  - `spender` cannot be the zero address.
    function increaseAllowance(Storage.Curve storage curve, address spender, uint256 addedValue) external returns (bool) {
        _approve(curve, msg.sender, spender, add(curve.allowances[msg.sender][spender], addedValue, "Curve/approval-overflow"));
        return true;
    }
    ...
    ///  `subtractedValue`.
    function decreaseAllowance(Storage.Curve storage curve, address spender, uint256 subtractedValue) external returns (bool) {
        _approve(curve, msg.sender, spender, sub(curve.allowances[msg.sender][spender], subtractedValue, "Curve/allowance-decrease-underflow"));
        return true;
    }
...
contract Curve is Storage, MerkleProver {
    ...
    function originSwap(address _origin, address _target, uint256 _originAmount, uint256 _minTargetAmount, uint256 _deadline) external deadline(_deadline) transactable() nonReentrant() returns (uint256 targetAmount_) {
    ...
    function viewOriginSwap(address _origin, address _target, uint256 _originAmount) external view transactable() returns (uint256 targetAmount_) {
    ...
    function targetSwap(address _origin, address _target, uint256 _maxOriginAmount, uint256 _targetAmount, uint256 _deadline) external deadline(_deadline) transactable() nonReentrant() returns (uint256 originAmount_) {
    ...
    function viewTargetSwap(address _origin, address _target, uint256 _targetAmount) external view transactable() returns (uint256 originAmount_) {
    ...
    function depositWithWhitelist(uint256 index, address account, uint256 amount, bytes32[] calldata merkleProof, uint256 _deposit, uint256 _deadline) external deadline(_deadline) transactable() nonReentrant() inWhitelistingStage() returns (uint256, uint256[] memory) {
        ...
        (uint256 curvesMinted_, uint256[] memory deposits_) = ProportionalLiquidity.proportionalDeposit(curve, _deposit);
    ...
    ///  @return (the amount of curves you receive in return for your deposit,
    ///           the amount deposited for each numeraire)
    function deposit(uint256 _deposit, uint256 _deadline) external deadline(_deadline) transactable() nonReentrant() notInWhitelistingStage() returns (uint256, uint256[] memory) {
    ...
    function emergencyWithdraw(uint256 _curvesToBurn, uint256 _deadline) external isEmergency() deadline(_deadline) nonReentrant() returns (uint256[] memory withdrawals_) {
    ...
    ///                         numeraire assets of the pool
    ///  @return withdrawals_ the amonts of numeraire assets withdrawn from the pool
    function withdraw(uint256 _curvesToBurn, uint256 _deadline) external deadline(_deadline) nonReentrant() returns (uint256[] memory withdrawals_) {
    ...
    function supportsInterface(bytes4 _interface) public pure returns (bool supports_) {
        supports_ = ((this.supportsInterface.selector == _interface) || (bytes4(0x7f5828d0) == _interface)) || (bytes4(0x36372b07) == _interface);
    }
    ...
    ///  @return success_ the success bool of the call
    function transferFrom(address _sender, address _recipient, uint256 _amount) public nonReentrant() returns (bool success_) {
