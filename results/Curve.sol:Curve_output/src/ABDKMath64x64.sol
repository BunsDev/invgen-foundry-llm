...
library ABDKMath64x64 {
    ...
    function divuu(uint256 x, uint256 y) public pure returns (uint128) {
        require(y != 0);
        uint256 result;
        if (x <= 0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF) result = (x << 64) / y; else {
            uint256 msb = 192;
            uint256 xc = x >> 192;
            if (xc >= 0x100000000) {
                xc >>= 32;
    ...
    function powu(uint256 x, uint256 y) public pure returns (uint256) {
        if (y == 0) return 0x80000000000000000000000000000000; else if (x == 0) return 0; else {
            int256 msb = 0;
            uint256 xc = x;
            ...
            if (xc >= 0x2) msb += 1;
            int256 xe = msb - 127;
            if (xe > 0) x >>= uint256(xe); else x <<= uint256(-xe);
            uint256 result = 0x80000000000000000000000000000000;
            int256 re = 0;
            while (y > 0) {
                if ((y & 1) > 0) {
            ...
            if (re > 0) result <<= uint256(re); else if (re < 0) result >>= uint256(-re);
            return result;
        }
    }
